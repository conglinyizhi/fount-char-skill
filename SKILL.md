...
当用户表示「想创建一个角色」且 AI 拥有向用户提问的能力（如 questionnaire 工具或对话式提问），请**额外读取 `character-wizard.md`**。该文件包含 7 步交互式问答流程，逐步引导用户从核心概念到台词风格完善角色，每步产出直接对应 fount 角色卡的具体文件。

### 非交互但用户发散模式

在没有 TUI 或问卷工具时，**额外读取 `interaction-style.md`** 切换为创意总监人设，以可爱活泼的语气用无序列表引导用户发散幻想。技术流程仍参考 `character-wizard.md`。
